...
- **Bugfix: void bets telden mee als verlies in win%/W-L/ROI (gemeld door gebruiker Robert Beune via Tom, 02-07-2026)**: "Als je een bet void, telt ie m mee als loss in win/loss en win%". Bevestigd als reëel en systemisch: tientallen bijna-identieke plekken in `streamlit_app.py` bouwden een "settled/afgerond"-lijst als `uitkomst in ("gewonnen","verloren","void")` — correct voor P&L (void boekt terecht €0), maar diezelfde lijst werd óók hergebruikt als noemer voor win% (`won / len(settled)`) en als basis voor het verliesaantal (`len(settled) − won`), waardoor elke void-bet ten onrechte als verlies meetelde. Ook de ROI-noemer (totaal ingezet) nam de void-inzet vaak ten onrechte mee, terwijl die stake wordt terugbetaald en dus niet "op het spel" stond. Fix: nieuwe centrale helper `_wl_stats(results)` in `streamlit_app.py` (net vóór `_parlay_leg_key()`) berekent won/lost/void/decided/win_pct/staked/pnl/roi met void correct uitgesloten van verliesaantal, win%-noemer én ROI-noemer (void blijft wel zichtbaar in "totaal gespeeld"-tellingen, want het is wel degelijk gebeurd). Toegepast op alle ~20 plekken in de app waar win%/W-L/ROI wordt getoond: Dashboard (all-time + maandweergave), Shortlist-samenvatting, Bankroll (7 dagen, maand/kalender, Overzicht, per-sport, Winst per odds-range, Model Prestaties rating-tiers, EV-kalibratie, modelnauwkeurigheid per sport, per bet-type, Parlay ROI), en Geplaatste Bets (totaal, per maand, per week, per-sport). Geverifieerd met een aparte grep-sweep dat geen enkele plek is gemist. **Werkt automatisch met terugwerkende kracht**: alle statistieken worden live berekend vanuit de database bij elke pagina-load (nooit vooraf berekend of opgeslagen), dus de fix corrigeert direct ook alle al bestaande, al afgeronde bets — geen data-migratie nodig.
- **Bugfix: verwijderde parlay liet een "spookrij" achter die bleef meetellen**: bij het onderzoeken van de void-bug ontdekt (zelfde categorie probleem — Tom's instructie "hetzelfde geldt voor evt verwijderde bets"). Een gesettelde parlay (gewonnen/verloren/void) krijgt een spiegel-rij in de `resultaten`-tabel (id `parlay_{id}`) zodat hij meetelt in de algemene statistieken. De "🗑️ Verwijder"-knop in 🎯 Parlay Builder → "Opgeslagen Parlays" verwijderde tot nu toe alleen de rij uit de `parlays`-tabel, niet deze spiegel-rij — waardoor een "verwijderde" parlay voor altijd bleef meetellen in win/loss/P&L/ROI door de hele app heen, als een onzichtbare wees-rij. Fix: de knop roept nu ook `db.remove_resultaat(f"parlay_{id}")` aan, net als de al langer bestaande verwijder-knop in 📋 Geplaatste Bets. Beide verwijder-knoppen in de app ruimen nu symmetrisch beide tabellen op.
...
- [x] **Void bets telden mee als verlies in win%/W-L/ROI** (gemeld door Robert Beune via Tom): opgelost via nieuwe centrale helper `_wl_stats()`, toegepast op ~20 plekken door de hele app (Dashboard, Shortlist, Bankroll, Geplaatste Bets). Werkt automatisch met terugwerkende kracht want alles wordt live uit de database berekend. Zie Key Features Built.
- [x] **Verwijderde parlay liet spookrij achter in resultaten-tabel**: "🗑️ Verwijder"-knop in Parlay Builder ruimt nu ook de `resultaten`-schaduwrij op (`db.remove_resultaat(f"parlay_{id}")`), net als de bestaande verwijder-knop in Geplaatste Bets. Zie Key Features Built.
